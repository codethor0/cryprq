...
mod kat_tests {
    ...
    fn test_kyber768_keypair_kat() {
        ...
        assert!(pk.as_bytes().iter().any(|&b| b != 0), "Public key must not be all zeros");
        assert!(sk.as_bytes().iter().any(|&b| b != 0), "Secret key must not be all zeros");
    }
    ...
    fn test_kyber768_encaps_decaps_kat() {
        ...
        assert_eq!(ss1.as_bytes(), ss2.as_bytes(), "Encaps/decaps shared secrets must match");
    }
    ...
    fn test_kyber768_deterministic_with_seed() {
        ...
        assert_ne!(pk1.as_bytes(), pk2.as_bytes(), "Different keypairs should differ");
        assert_ne!(sk1.as_bytes(), sk2.as_bytes(), "Different keypairs should differ");
    ...
    fn test_kyber768_roundtrip_correctness() {
        ...
        let (pk, sk) = keypair();
        let (ct, ss_encaps) = encapsulate(&pk);
